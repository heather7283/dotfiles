...
if [ ! -f ~/.config/dosbox/dosbox-SVN.conf ]; then
    mkdir -pv ~/.config/dosbox || die "can't create config dir"
    echo "$default_config" >~/.config/dosbox/dosbox-SVN.conf || die "can't write default config"
fi

set -- -conf ~/.config/dosbox/dosbox-SVN.conf "$@"
